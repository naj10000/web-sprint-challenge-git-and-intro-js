...
function lotsOfArt(array) {
  let newArray = []
   for(let i = 0; i < array.length; i++){
    array[i].paintings > 100 ? newArray.push(array[i].name) : null
   }
...
function artistInfo(array, name){
  let bio
  for(let i= 0; i<array.length; i++){
    array[i].name ===name ? bio = array[i].bio : null
...
function artistByCountry(array, nationality){
  ...
  for(let i = 0; i< array.length; i++){
    array[i].nationality === nationality ? newArray.push(array[i].name) : null
  }
  return newArray
}
